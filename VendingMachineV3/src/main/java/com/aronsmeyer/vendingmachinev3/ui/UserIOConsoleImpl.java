...
public class UserIOConsoleImpl implements UserIO {
    ...
    public BigDecimal readBigDecimal(String msgPrompt) {
        while(true) {
            System.out.println(msgPrompt);
            // convert String input to BigDecimal (nextBigDecimal inserts extra return)
            try {
                Scanner sc = new Scanner(System.in);
                String inputStringAmt = sc.nextLine();
                BigDecimal inputAmt = new BigDecimal(inputStringAmt);
                BigDecimal inputMoney = inputAmt.setScale(2, RoundingMode.HALF_UP);
                return inputMoney;
            } catch (NumberFormatException e) {
                this.print("Input error. Please try again.");
            }
